...
Let $U$ be the collection of all $a^\ast \Cone Q$ among $\Cone Q \to \Cone P$ in $R$ such that $P^{\rm gp} \to Q^{\rm gp}$ is surjective.  This collection is finite, consists only of subfunctors of the functor represented by $\Cone P$, and stable under fiber products (intersections of functors), so for every point $y$ of $|R_X|$ there is an initial $a^\ast \Cone Q$ in $U$ and factorization $y \to a^\ast \Cone Q$.  Therefore there is a deformation retraction of $|R_X|$ onto $|U|$.  It will suffice to show that $|U|$ is contractible.

We argue now that $|U|$ is homotopy equivalent to $|U^\circ|$ where $U^\circ$ consists only of those $a^\ast \Cone Q$ such that $

Suppose that $y \in |Y_i|$.  We give $y$ the logarithmic structure restricted from $Y_i$.  If $Q_i \to \overnorm M_y$ is not sharp (so $y$ is not in the closed stratum of $Y_i$) then $Q_i^{\rm gp} \to \overnorm M_y^{\rm gp}$ has a nontrivial kernel.  By induction on the rank of $\overnorm M_X$, we can assume that the groupoid $|Y_\bullet \mathop\times_X y|$ is contractible.  Repeating if necessary, we can replace $y$ by a $y'$ from a contractible space of choices such that $y' \in |Y_j|$ and $Q_i \to \overnorm M_{y'}$ is sharp.
...
\begin{lemma} \label{lem:etale-descent}
Suppose we have a family $Z_i \to Y_i$ of quasicompact, quasiseparated, \'etale morphisms, funtorial in $Y_i$.  Then there is a unique quasicompact, quasiseparated, \'etale map $Z \to X$ such that $Z_i = Z \mathop\times_{X} Y_i$.
\end{lemma}
\begin{proof}
For each $i$, there is a minimal stratification of $Y_i$ such that the restriction of $Z_i$ to each stratum is finite \'etale.  This stratification commutes with base change.  Lemmas~\ref{lem:submersive} and~\ref{lem:top-gpd} show that the underlying topological space of $X$ is the quotient of the underlying topological spaces of the $Y_i$.  Therefore this stratification descends to a stratification of $X$.
